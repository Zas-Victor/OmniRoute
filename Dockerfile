...
RUN --mount=type=cache,id=omniroute-apt-cache,target=/var/cache/apt,sharing=shared \
  --mount=type=cache,id=omniroute-apt-lists,target=/var/lib/apt/lists,sharing=shared \
  apt-get update \
  && apt-get upgrade -y \
