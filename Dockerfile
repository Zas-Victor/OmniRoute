...
RUN --mount=type=cache,id=cacheKey-omniroute-apt-cache,target=/var/cache/apt,sharing=shared \
...
RUN --mount=type=cache,id=cacheKey-omniroute-npm-cache,target=/root/.npm \
...
COPY . ./
RUN --mount=type=cache,id=cacheKey-omniroute-next-cache,target=/app/.build/next/cache \
...
RUN --mount=type=cache,id=cacheKey-omniroute-apt-cache,target=/var/cache/apt,sharing=locked \
  --mount=type=cache,id=cacheKey-omniroute-apt-lists,target=/var/lib/apt/lists,sharing=locked \
  apt-get update \
